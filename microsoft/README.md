...
| PKCE for non-PKCE connectors | Done — handled by the authorization server, so the connector needs no PKCE configuration. See "Authentication notes". |
...
- **Staged for Partner Center:** the submission zip is held in private release storage. Mint a short-lived read-only SAS at submission time and paste it straight into Partner Center. The account, container, blob name and exact command live in the internal runbook, not in this public repo.

  ⚠ **Never commit a minted SAS.** This repository is public, so a committed SAS is a live credential the moment it lands.
...
- **PKCE is handled for you.** Supabase GoTrue (OAuth 2.1) mandates PKCE, but Power Platform's generic OAuth2 connector does not send `code_challenge`/`code_verifier`, so consent used to fail with *"PKCE flow requires both code_challenge and code_challenge_method"*. The authorization server resolves this server-side, so the connector needs no PKCE configuration and nothing to set here. Public clients such as ChatGPT, Claude and Cursor send their own PKCE and are unaffected.
