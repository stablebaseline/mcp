...
180 tools across 18 categories. Same brain, same Knowledge Graph, same RBAC — every surface.
...
180 MCP tools across 18 categories let an agent drive **end-to-end**: sign-up → billing → org/workspace/project → members → teams → permissions → brain scope → docs → diagrams → plans, all without a human touching the UI.
...
- 🟢 **Smithery**: published with `configSchema` (Quick Setup). 180 tools / 8 prompts / 6 resources discovered.
